...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        .setTitle("Astronaut Bot")
        .setDescription(`You have been muted on the Lunar Discord by ${message.author} for \`${time}\` ` + minutes + ` with reason \`${reason}\`.\n\n**Attempting to bypass a mute will get you muted for a longer time if you do so.**`)
        .setColor(0x3c368f)
        .setFooter(message.id)
        .setTimestamp();
        ...
        .setTitle("Astronaut Bot")
        .setDescription(`You have been unmuted on the Lunar Discord automatically by the bot.`)
        .setColor(0x3c368f)
        .setFooter(message.id)
        .setTimestamp();
